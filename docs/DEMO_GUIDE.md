...
- 핵심 근거·리스크 트리거 불릿
- **ETF (`SPY`)**: 보수·회전율·AUM, 상위 보유 칩, 섹터 비중 바 — 목표가/손절 대신 펀드 지표 중심

**한 줄 멘트**: 「LLM 리포트를 파싱해 의사결정에 필요한 숫자만 카드로 올렸습니다.」

### 2b. ETF 리포트 탭 (`#SPY/2026-06-08`)

- 사이드바 **SPY** 선택 → **리포트** 탭
- MD 6섹션(요약·보유·운용비용·리스크·모멘텀·전략)을 표·콜아웃·불릿으로 구조화
- 출처는 `Yahoo Finance ETF profile` 등 **사용자 친화 라벨** (내부 `provided input`은 파이프라인에서 자동 정규화)

**한 줄 멘트**: 「ETF는 주식과 다른 6섹션 계약이고, UI도 보유·비용·괴리 리스크에 맞춰 따로 렌더합니다.」
