...
| 스크립트 | 역할 |
|----------|------|
| `scripts/run_dashboard.py` | FastAPI 웹 데모 — 티커 입력·Job 실행·탭별 산출물 조회 (`src/web/`) |
...
**웹 데모 대시보드** (`uv run scripts/run_dashboard.py`, 기본 `http://127.0.0.1:8765/`): 브라우저에서 티커·날짜를 넣고 Job으로 `run_single_pipeline()`을 호출합니다. 진행 로그는 폴링으로 표시되고, 완료 후 개요·데이터·뉴스·컨텍스트·리포트·평가·신호 탭에서 JSON/Markdown을 확인할 수 있습니다. 과거 실행은 `/api/history`로 나열된 `artifacts/<티커>/<날짜>/`를 다시 로드합니다.

- **인터넷 공개**: `brew install cloudflared` 후 `uv run scripts/run_dashboard.py --public` → 콘솔에 `https://….trycloudflare.com` URL 출력. 터미널을 끄면 URL도 무효화됩니다.
- **결과 공유**: 브라우저 주소 `#AAPL/2026-06-08` 해시로 특정 산출물 링크 공유 가능.
